...
//   putchar(toupper(sta[nsta].n));
  
      for (k=0; k<=nst; k++) {
		if (strcmp(st[k],sta[nsta].n)==0){  
          if ((sta[nsta].lo>=lon1)&&(sta[nsta].lo<=lon2)&&
          (sta[nsta].la>=lat1)&&(sta[nsta].la<=lat2))
...
/*int fils(char *s) {
  int i;
  for (i=0; i<nsta; i++) {
    if (strncasecmp(s,sta[i].n,4)==0) {
      if ((sta[i].lo>=lon1)&&(sta[i].lo<=lon2)&&
        (sta[i].la>=lat1)&&(sta[i].la<=lat2)) return(0); 
      else return(1);
    } 
  }
  printf("fils, unknown station %s\n",s);
  return(1);
}*/
